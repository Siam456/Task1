...
const Main = () => {
	...
	useEffect(() => {
		//console.log('axios')
		if (obj.value !== 0 && obj.ExchangeRate !== 0 && obj.productID !== null) {
			//console.log(obj + '');
			axios
				.get(`/api/${obj.productID}`)
				.then((res) => {
					console.log(res);
					setproduct({
						...
						SDTK: (
							((obj.AV +
								(res.data.products[0].CD / 100) +
								(res.data.products[0].RD / 100)) *
								(res.data.products[0].SD) /
							100)
						).toFixed(2),
						VATTK: (
							((obj.AV +
								...
								((obj.AV +
									...
									(res.data.products[0].SD) /
									100)) *
								(res.data.products[0].VAT) /
							100)
						).toFixed(2),
						RDTK: ((obj.AV * (res.data.products[0].RD) / 100)).toFixed(2),
						total: (
							(obj.AV * (res.data.products[0].CD) / 100) +
							((obj.AV +
								(res.data.products[0].CD / 100) +
								(res.data.products[0].RD / 100)) *
								(res.data.products[0].SD) /
	...
	const changeData = (e) => {
		...
		if (name === "cd") {
			...
			setproduct((prev) => {
				...
				const sd = (obj.AV + cd + rd) * (prev.SD / 100);
				const vat = (obj.AV + cd + rd + sd) * (prev.VAT / 100);
			...
			setproduct((prev) => {
				const cd = obj.AV * (prev.CD / 100);
				const rd = obj.AV * (prev.RD / 100);
				const sd = (obj.AV + cd + rd) * (newValue / 100);
				const vat = (obj.AV + cd + rd + sd) * (prev.VAT / 100);
				const total = obj.AV + cd + rd + sd + vat;
				//console.log(product.SDTK)
				return {
			...
			setproduct((prev) => {
				...
				const rd = obj.AV * (prev.RD / 100);
				const sd = (obj.AV + cd + rd) * (prev.SD / 100);
				const vat = (obj.AV + cd + rd + sd) * (newValue / 100);
				const total = obj.AV + cd + rd + sd + vat;
				//console.log(product.SDTK)
				return {
					productID: prev.productID,
					...
					<div className="grid-item">
						...
						>
							...
							<input
								...
								onChange={setValue}
								type="number"
								value={obj.ExchangeRate === 0 ? "" : obj.ExchangeRate}
							/>
						</div>
						<div className="flexAndSpaceBetween">
					...
					<div style={{ display: "block" }}>
						<div className="insuranceAndChargeWrapper">
							<div className="flexAndSpaceBetween">
								...
								<span>
									{obj.insurance === 0 ? obj.insurance + ".00" : obj.insurance}
									USD
								</span>
							</div>
							<div className="flexAndSpaceBetween">
								<span>Charge 1%</span>
								<span>
									{obj.Charge === 0 ? obj.Charge + ".00" : obj.Charge}USD
								</span>
							</div>
						</div>
						<div>
						...
						<div className="insuranceAndChargeWrapper">
							<div className="flexAndSpaceBetween">
								...
								<input
									...
									value={`${
										obj.AV === 0 ? obj.AV + ".00" : obj.AV.toFixed(2)
									}TK`}
								/>
							</div>
							<div className="flexAndSpaceBetween">
								...
								<input
									style={{ textAlign: "right" }}
									value={`${
										product.CDTK === 0 ? product.CDTK + ".00" : product.CDTK
									}TK`}
								/>
							</div>
							<div className="flexAndSpaceBetween">
								...
								<input
									style={{ textAlign: "right" }}
									value={`${
										product.RDTK === 0 ? product.RDTK + ".00" : product.RDTK
									}TK`}
								/>
							</div>
							<div className="flexAndSpaceBetween">
								...
								<input
									style={{ textAlign: "right" }}
									value={`${
										product.SDTK === 0 ? product.SDTK + ".00" : product.SDTK
									}TK`}
								/>
							</div>
							<div className="flexAndSpaceBetween">
								<div>
									<span>VAT</span>
									<span style={{ marginLeft: "22px" }} className="inputField">
										<input
											autoComplete="off"
											type="number"
											onChange={changeData}
								...
								<input
									style={{ textAlign: "right" }}
									value={`${
										product.VATTK === 0 ? product.VATTK + ".00" : product.VATTK
									}TK`}
								/>
							</div>
						</div>
						...
						<div>
							<div className="flexAndSpaceBetween">
								<span>Total</span>
								<span>
									{product.total === 0 ? product.total + ".00" : product.total}
									TK
								</span>
							</div>
						</div>
